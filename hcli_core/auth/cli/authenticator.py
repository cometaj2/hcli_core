...
# Deny access if authentication is disabled
def deny_disabled_authentication(func):
    @wraps(func)
...
class AuthorizationMiddleware:
    ...
    def process_request(self, req: falcon.Request, resp: falcon.Response):
        if self.cfg.auth:
            client_ip = self.get_client_ip(req)
            if not self.is_authorized(req, resp, client_ip):
                resp.append_header('WWW-Authenticate', 'Basic realm="default"')
                raise AuthorizationError(detail="invalid credentials provided", instance=req.path)
    ...
    def log_failed_attempt(self, ip, username=None):
        ...
        # Log the failed attempt
        log_message = f'Failed authorization attempt from IP: {ip}'
        if username:
            log_message += f' (username: {username})'
        log.warning(log_message)

    def is_authorized(self, req: falcon.Request, resp: falcon.Response, client_ip) -> bool:
        authorized = False
        ...
        if auth_type.lower() == 'basic':
            decoded = base64.b64decode(auth_string).decode('utf-8')
            username, password = decoded.split(':', 1)
            authorized = self.cm.validate_basic(username, password)
            ...
            prefix, leftover = apikey.split('_', 1)
            if prefix == 'hcoak':
                authorized = self.cm.validate_hcoak(keyid, apikey)
                if not authorized:
                    msg = 'invalid credentials for keyid: ' + keyid + "."
                    self.log_failed_attempt(client_ip)
                    log.warning(msg)
...
class SelectiveAuthorizationMiddleware(AuthorizationMiddleware):
    ...
    def process_resource(self, req, resp, resource, params):
        ...
        if getattr(resource, 'requires_authorization', False):
            log.debug("Resource requires authorization, authorizing...")
            super().process_request(req, resp)
        else:
...
class HCLIAuthorizationMiddleware:
    def __init__(self, name):
        self.cfg = config.Config(name)
...
class SelectiveHCLIAuthorizationMiddleware(HCLIAuthorizationMiddleware):
    def __init__(self, name):
        super().__init__(name)
